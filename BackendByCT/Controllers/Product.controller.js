...
const getProductBySearch = (req, res) => {
    ...
    const filteredArray = productData.filter((data) => data.title.trim().toLowerCase().includes(searchQuery),);
...
const getProductByCategory = (req, res) => {
    ...
    const filteredArray = productData.filter((data) => data.category.trim().toLowerCase().includes(category),);
    res.send(filteredArray);
...
const deleteProduct = (req, res) => {
    const productId = req.params.id;
    const removeData = productData.splice(productId - 1, 1);
    res.send(removeData);
}

export { getProductById, getProductBySearch, getProductByCategory, getProducts, createProduct, modifyProduct, replaceProduct, deleteProduct };